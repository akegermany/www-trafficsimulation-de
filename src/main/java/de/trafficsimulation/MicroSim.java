...
import de.trafficsimulation.core.AdjustmentMethods;
...
public class MicroSim extends Applet implements Constants {
    ...
    private double qIn = (CHOICE_SCEN_INIT == 2) ? Q_INIT2 / 3600. : (CHOICE_SCEN_INIT == 3) ? Q_INIT3 / 3600.
    ...
    final Color BACKGROUND_COLOR = new Color((float) (0.8), (float) (0.8), (float) (0.8));
    ...
    final Color BUTTON_COLOR = new Color((float) (0.8), (float) (0.8), (float) (0.8));
    ...
    private SimCanvas simCanvas; // =new SimCanvas (2, density, qIn, perTr, p_factor, deltaB, p_factorRamp, deltaBRamp, tsleep_ms);//
                                 // simulation field
    ...
    public void init() {
        ...
        simCanvas = new SimCanvas(choice_Scen, density, qIn, perTr, p_factor, deltaB, p_factorRamp, deltaBRamp,
                tsleep_ms);// simulation field

        if (choice_Scen != 2) {
            simCanvas.newValues(choice_Scen, density, qIn, perTr, v0_limit, p_factor, deltaB, tsleep_ms);
        } else {
            simCanvas.newValues2(qIn, perTr, p_factor, deltaB, qRamp, p_factorRamp, deltaBRamp, tsleep_ms);
        }
        // ############################################
        ...
        sb_deltaBRamp = getSB((int) (100 * DELTABRAMP_MIN), (int) (100 * DELTABRAMP_MAX), deltaB100Ramp);
        ...
        pClosedSystem.add(label_perTr1 = new Label(String.valueOf((int) (100 * FRAC_TRUCK_INIT_CIRCLE)) + " %"),
                gbconstr);
        ...
        pRamp.add(label_perTr2 = new Label(String.valueOf(truckPerc) + " %"), gbconstr);
        ...
        pSource.add(label_p_factor35 = new Label(String.valueOf(p_factor)), gbconstr);
    ...
    private void makeGlobalLayout() {
        ...
        System.out.println("MicroSim.makeGlobalLayout:" + "Applet-size=" + (getSize().width) + " X "
                + (getSize().height) + ", Client-size=" + usedWidth + " X " + usedHeight);
        ...
        int sbWidth = isCircular ? (int) (0.70 * simSize) // weiter unten auch veraendern!
                : (int) (0.7 * usedWidth);
        int sbHeight = isCircular ? (int) (0.16 * simWinHeight) : (int) (0.22 * simWinHeight);
        ...
        int sbTop = isCircular ? simWinTop + (int) (0.5 * (simSize - 1.2 * sbHeight - sb_IDMheight)) : simWinTop
                + (int) (0.5 * (simSize - 1.2 * sbHeight - sb_IDMheight)) - (int) (0.2 * sbHeight);
    ...
    public boolean handleEvent(Event evt) {
        ...
        case Event.SCROLL_ABSOLUTE:
            ...
            // martin jan05
                ...
                double newval = AdjustmentMethods.getVariableFromSliderpos(sb_IDMv0.getValue(), V0_MIN_KMH, V0_MAX_KMH,
                        false);
                ...
                idmCar.a = AdjustmentMethods
                        .getVariableFromSliderpos(sb_IDMa.getValue(), A_MIN_MSII, A_MAX_MSII, false);
                ...
                idmCar.b = AdjustmentMethods
                        .getVariableFromSliderpos(sb_IDMb.getValue(), B_MIN_MSII, B_MAX_MSII, false);
