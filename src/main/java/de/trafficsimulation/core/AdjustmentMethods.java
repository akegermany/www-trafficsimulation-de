...
public class AdjustmentMethods implements Constants {
    ...
    public static double changeVarByIncr(double incr, int rangePix, double var, double varMin, double varMax,
            boolean logarithmic) {
        double arg = var;
        ...
        double relpos = (logarithmic) ? (Math.log(arg) - lnMin) / (lnMax - lnMin) : (arg - varMin) / (varMax - varMin);

        double relposNew = relpos + incr / (double) rangePix;
        relposNew = Math.max(Math.min(relposNew, 1), 0);

        double result = (logarithmic) ? Math.exp(lnMin + relposNew * (lnMax - lnMin)) : varMin + relposNew
                * (varMax - varMin);

        if (false) {
            System.out.println("changeVarByIncr: var=" + var + " relpos=" + relpos + " relposNew=" + relposNew
                    + " result=" + result);
        }
        return result;
    }
    ...
    // ################################################
    public static double getVariableFromRel(double relpos, double varMin, double varMax, boolean logarithmic) {
        if (logarithmic) {
    ...
    public static double getVariableFromSliderpos(int sliderPos, double varMin, double varMax, boolean logarithmic) {
    ...
    public static int getSliderposFromVariable(double var, double varMin, double varMax, boolean logarithmic) {
        final int SLIDER_STEPS = 1000;
        double relpos = 0;
        double arg = var;
        if (arg < Math.min(varMin, varMax)) {
        ...
        if (logarithmic) {
            ...
            if (false) {
                System.out.println("!!!getSliderposFromVariable: lnMin=" + lnMin + " lnMax=" + lnMax + " lnarg="
                        + Math.log(arg) + " var=" + var + " arg=" + arg + " varMin=" + varMin + " varMax=" + varMax
                        + " relpos=" + relpos);
            }
        }
    ...
    public static void addScrollbar(Panel panel, int row, Scrollbar sb, String str_quantity, Label changeableLabel) {
    ...
    public static Scrollbar getSB(double min, double max, double init, boolean logarithmic) {
        final int knobSize = 1;
        final int SLIDER_STEPS = 1000;

        int initPos = getSliderposFromVariable(init, min, max, logarithmic);
        Scrollbar sb = new Scrollbar(Scrollbar.HORIZONTAL, initPos, knobSize, 0, SLIDER_STEPS);
        ...
        if (false) {
            System.out.println("getSB: init=" + init + " min=" + min + " max=" + max + " init=" + init + " initPos="
                    + initPos);
        }
        return sb;
    }
