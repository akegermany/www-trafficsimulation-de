...
public class OnRamp extends MicroStreet {
    ...
    public OnRamp(MicroStreet mainroad, double totalLength, double rampLength, double mergingPosAtMainroad,
        ...
        street.addElement(new Car(5., idmCar.Veq(roadLength), 0, idmCar, inconsiderate, PKW_LENGTH_M, colorCar, 2));
    }

    public void update(double time, double dt, double qIn, double perTr, double p_factorRamp, double deltaBRamp) {
    ...
    private void mergeToMainRoad(LaneChange laneChangeModel, double mergingPos) {
        ...
        if (false) {
            System.out.println("in mergeToMainRoad: " + "1 obstacle + " + (imax) + " veh(s) on ramp, " + nvehmain
                    + " on mainroad;" + " offset=" + (int) (offsetMain));
        }
                    ...
                    if (me.change(frontVeh, frontVehMain, backVehMain)) {
                        ...
                        if (false) {
                            System.out.println(" mergeToMainRoad: Changing!!!\n " + "  deltaBRamp="
                                    + ((Car) me).lanechange.get_db() + "  nvehmain=" + mainroad.street.size()
    ...
    private void setNeighboursOnMainRoad(int i, double offsetMain) {
        ...
        if (nvehmain > 0) {
            ...
            for (imain = 0; ((imain < nvehmain) && (x < xmain)); imain++) {
                ...
                if (debug) {
                    System.out.println("OnRamp:setNeighbours..for loop: imain=" + imain + " xmain=" + ((int) (xmain))
                            + " nvehmain=" + nvehmain);
                }
            }
            ...
            if (debug) {
                System.out.println("i_frontmain(right lane)=" + i_frontmain + " lanemain=" + lanemain);
            }
            ...
            if (debug) {
                System.out.println("i_backmain(right lane)=" + i_backmain + " lanemain=" + lanemain);
            }

            if (debug) {
                if (i_frontmain == -1)
                    System.out.println("OnRamp:setNeighbours: No front vehicle, but >=1 back veh!");
                if (i_frontmain + 1 == nvehmain)
                    System.out.println("OnRamp:setNeighbours: No back vehicle, but >=1 front veh!");
            }
        }

        else { // nvehmain=0
            if (debug) {
                System.out.println("OnRamp:setNeighbours: nvehmain=" + nvehmain + " => no vehicle on main road!");
            }
            i_frontmain = -1;
            i_backmain = -1;
        }
        ...
        ? new Car(farDistance, 0, 0, idmCar, inconsiderate, 5, colorCar, 0)
                : new Car((Car) (mainroad.street.elementAt(i_frontmain)));

        backVehMain = ((nvehmain < 1) || (i_backmain >= nvehmain)) ? new Car(-farDistance, 0, 0, // only front veh(s) or none
                idmCar, inconsiderate, 5, colorCar, 0) : new Car((Car) (mainroad.street.elementAt(i_backmain)));
        if (debug) {
            System.out.println(" setNeighboursOnMainRoad!!!:" + " nvehmain=" + nvehmain + " i_backmain=" + i_backmain
        ...
        if (debug) {
            System.out.println(" setNeighboursOnMainRoad, got neighbours:" + "\n   i_frontmain=" + i_frontmain
                    + ", x_front=" + frontVehMain.position() + ", x_back=" + backVehMain.position() + " offsetMain="
                    + offsetMain);
        }
