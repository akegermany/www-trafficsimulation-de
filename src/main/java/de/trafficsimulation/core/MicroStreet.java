...
public class MicroStreet implements Constants {
    ...
    public MicroStreet(double length, double density, double p_factor, double deltaB, int floatcar_nr, int choice_Szen) {
        time = 0;
        roadLength = length;
        ...
        if (choice_Szen == 1) {
            ...
            for (int i = 0; i < nCars; i++) {
                int lane = (i % 2 == 0) ? 1 : 0;
                street.insertElementAt(new Car(roadLength - (i + 1) * distance, vNew, lane, idmCar, polite,
                        PKW_LENGTH_M, colorCar, i), i);
        ...
        if (choice_Szen == 6) {
            ...
            for (int i = 0; i < nCars; i++) {
                ...
                if (i % 7 == 0) {
                    if (i != 49)
                        street.insertElementAt(new Obstacle(roadLength - (i + 1) * 20.0, lane, 5., 9999), index);
                    if (i == 49)
                        street.insertElementAt(new Obstacle(roadLength + 30.0 - (i + 1) * 20.0, lane, 5., 9999), index);
                } else {
                    if (i < ((int) (0.5 * nCars))) {
                        street.insertElementAt(new Car(roadLength - (i + 1) * 20.0, 0.0, lane, idmCar, inconsiderate,
                                PKW_LENGTH_M, colorCar, i), index);
                    } else {
                        street.insertElementAt(new Car(roadLength - (i + 1) * 20.0, 0.0, lane, idmCar, inconsiderate,
                                PKW_LENGTH_M, Color.green, i), index);
                    }
                }
                if (i != 49) {
                    index++;
        ...
        if (choice_Szen == 5) {
            ...
            street.addElement(new Car(5.0, idmCar.Veq(roadLength), 0, idmCar, polite, PKW_LENGTH_M, colorCar, 2));
        }
        ...
        if (choice_Szen == 3) {
            ...
            for (int i = 0; i < n_obst; i++) {
                ...
                street.insertElementAt(new Obstacle(pos, 0, obst_elem_length + 1, i), i);
            }
            street.insertElementAt(new Car(5.0, idmTruck.Veq(roadLength), 0, idmTruck, polite, LKW_LENGTH_M,
                    colorTruck, n_obst), n_obst);
        }
        ...
        if (choice_Szen == 4) {
            ...
            street.insertElementAt(new Car(5.0, sync1Car.Veq(roadLength), 0, sync1Car, polite, PKW_LENGTH_M, colorCar,
                    0), 0);
        }
    ...
    public void update(double time, double dt, int choice_Szen, double density, double qIn, double perTr,
    ...
    private void insertOneVehicle(double perTr) {
        ...
        System.out.println("MicroStreet.insertOneVehicle: maxgap=" + (int) maxgap + " index=" + i_maxgap + " pos="
                + pos_maxgap + " lane=" + lane_maxgap);
        ...
        if (maxgap > mingap) {
            ...
            street.insertElementAt((new Car(posNew, vNew, lane_maxgap, modelNew, changemodelNew, lNew, colorNew,
                    randInt)), i_maxgap);
        }
    ...
    protected void insertBCCars(int choice_BC) {
        ...
        double downLeftVel = (i_ld > -1) ? ((Moveable) (street.elementAt(i_ld))).velocity() : 0;
        double downRightVel = (i_rd > -1) ? ((Moveable) (street.elementAt(i_rd))).velocity() : 0;
            ...
            if (nin > 1.0) { // new vehicle imposed by the inflow BC
                ...
                if ((i_lu >= 0) && (i_ru >= 0)) {
                    ...
                    if (!(red = (space < spaceMin))) {
                        ...
                        if (false) {
                            System.out.println("MicroStreet.ioFlow: " + ((rand < perTr) ? "Truck" : "Car") + ", vNew="
                                    + vNew);
                        }
                        double lNew = (rand < perTr) ? LKW_LENGTH_M : PKW_LENGTH_M;
                        Color colorNew = (rand < perTr) ? colorTruck : colorCar;
                        ...
                        imax = street.size();
                        street.insertElementAt(new Car(0.0, vNew, lane, modelNew, changemodelNew, lNew, colorNew,
                                randInt), imax);
                    }
                }
                ...
                // at least one lane without vehicles
                    ...
                    street.insertElementAt(new Car(0.0, vNew, lane, modelNew, inconsiderate, lNew, colorNew, randInt),
                            imax);
                }
            }
        }
